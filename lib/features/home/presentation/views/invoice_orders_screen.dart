  ...
  Widget build(BuildContext context) {
    return BlocListener<ChangeStatusCubit,ChangeStatusState>(
      listener: (context,state){
        if(state is ChangeStatusDone){
          if(state.type == 'Partial'){
            ...
            context.pushNamed(AppRouteConstants.runSheetInvoicesRouteName,extra: [widget.runSheetItemEntity.runSheetId]);
          }
      ...
      child: Scaffold(
        ...
        body: BlocBuilder<OrdersCubit, OrdersState>(builder: (context, orderState) {
          if (orderState is OrdersLoaded) {
            return Column(
              children: [
                ...
                if(widget.runSheetItemEntity.status == 'Created')
                Container(
                  height: 60,
                              ...
                              onPressed: () async {
                                ...
                                if (result != null && context.mounted) {
                                  ...
                                  BlocProvider.of<ChangeStatusCubit>(context,listen: false)
                                      ...
                                      status: 'Delivered', invoiceId: widget.runSheetItemEntity.id.toString());
                                }
                              },
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () async{
                                int? result =
                                    await CoreUtils.showInvoiceConfirmationDialog(
                                    context,
                                    S.of(context).partial_delivery_confirmation);
                                if (result != null && context.mounted) {
                                  final token =
                                      context.read<UserProvider>().user!.token;
                                  final lang = context
                                  ...
                                  BlocProvider.of<ChangeStatusCubit>(context,listen: false)
                                      .changeOrderStatus(token: token!, lang: lang,
                                      orders: orderState.orders,
                                      status: 'Partial', invoiceId: widget.runSheetItemEntity.id.toString());
                                }
                              },
                              ...
                              onPressed: () async {
                                ...
                                if (result != null && context.mounted) {
                                  ...
                                  BlocProvider.of<ChangeStatusCubit>(context,listen: false)
                                      .changeOrderStatus(token: token!, lang: lang,
                                      orders: orderState.orders,
                                      status: 'Refused', invoiceId: widget.runSheetItemEntity.id.toString());
                                }
                              },
